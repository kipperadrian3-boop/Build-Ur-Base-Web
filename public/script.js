document.addEventListener('DOMContentLoaded', () => {
    ...
    const topLeaderboardBtn = document.getElementById('topLeaderboardBtn');
    const leaderboardModal = document.getElementById('leaderboardModal');
    const closeLeaderboardBtn = document.getElementById('closeLeaderboardBtn');
    const leaderboardList = document.getElementById('leaderboardList');
    const lbTabBtns = document.querySelectorAll('.lb-tab-btn');

    let currentLbBoard = 'coins';
    ...
    if (topLeaderboardBtn) {
        topLeaderboardBtn.addEventListener('click', openLeaderboardModal);
    }
    ...
    lbTabBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            currentLbBoard = btn.getAttribute('data-board') || 'coins';
            renderLeaderboard();
        });
    });
    ...
    async function fetchLeaderboard() {
        ...
        if (!cachedLeaderboardData) {
            leaderboardList.innerHTML = '<div class="lb-loading">Loading rankings...</div>';
        }

        try {
            const res = await fetch('https://build-ur-base-web.onrender.com/api/leaderboard');
            const data = await res.json();
            ...
            if (!cachedLeaderboardData) {
                leaderboardList.innerHTML = '<div class="lb-empty">Could not load leaderboard. Please try again.</div>';
            }
        }
    }

    function renderLeaderboard() {
        if (!leaderboardList || !cachedLeaderboardData) return;
        leaderboardList.innerHTML = '';

        const list = cachedLeaderboardData[currentLbBoard] || [];
        if (list.length === 0) {
            leaderboardList.innerHTML = '<div class="lb-empty">No players on the leaderboard yet!</div>';
            return;
        }

        list.forEach(player => {
            const isMe = currentUser && (String(player.userId) === String(currentUser.userId));
            const item = document.createElement('div');
            item.className = `lb-item${isMe ? ' is-me' : ''}`;

            let rankDisplay = `#${player.rank}`;
            let rankClass = '';
            if (player.rank === 1) { rankDisplay = '🥇'; rankClass = 'rank-1'; }
            else if (player.rank === 2) { rankDisplay = '🥈'; rankClass = 'rank-2'; }
            else if (player.rank === 3) { rankDisplay = '🥉'; rankClass = 'rank-3'; }

            let valueDisplay = '';
            let badgeDisplay = '';
            if (currentLbBoard === 'coins') {
                valueDisplay = `${(player.value || 0).toLocaleString('de-DE')} 🪙`;
                badgeDisplay = 'Coin Balance';
            } else if (currentLbBoard === 'waves') {
                valueDisplay = `Wave ${player.value || 1} 🌊`;
                badgeDisplay = 'Survival Wave';
            } else if (currentLbBoard === 'streak') {
                valueDisplay = `${player.value || 1} Days 🔥`;
                badgeDisplay = 'Daily Streak';
            }

            const avatarSrc = player.avatarUrl || 'https://tr.rbxcdn.com/38c6edcb50633730ff4cf39ac8859840/150/150/AvatarHeadshot/Png';

            item.innerHTML = `
                <div class="lb-rank ${rankClass}">${rankDisplay}</div>
                <img src="${avatarSrc}" alt="${player.username}" class="lb-avatar" onerror="this.src='https://tr.rbxcdn.com/38c6edcb50633730ff4cf39ac8859840/150/150/AvatarHeadshot/Png'">
                <div class="lb-info">
                    <div class="lb-name">
                        ${player.username}
                        ${isMe ? '<span class="lb-you-tag">YOU</span>' : ''}
                    </div>
                    <span class="lb-badge">${badgeDisplay}</span>
                </div>
